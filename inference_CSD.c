...
#include "model_params_CSD.h"   // CSD 3-term requant parameters
...
typedef struct {
    const int8_t  *W;
    const int32_t *b;
    const int8_t  (*csd_sign)[CSD_TERMS];
    const int8_t  (*csd_shift)[CSD_TERMS];
    const int32_t *shift;
    int32_t Zx;
    int32_t Zout;
} LayerInt8;
...
static Model load_model(void)
{
    ...
    m.conv1.W = c0_w;
    m.conv1.b = c0_b;
    m.conv1.csd_sign = c0_csd_sign;
    m.conv1.csd_shift = c0_csd_shift;
    m.conv1.shift = c0_s;
    m.conv1.Zx = c0_Zx;
    m.conv1.Zout = c0_Zo;
    ...
    m.conv2.W = c1_w;
    m.conv2.b = c1_b;
    m.conv2.csd_sign = c1_csd_sign;
    m.conv2.csd_shift = c1_csd_shift;
    m.conv2.shift = c1_s;
    m.conv2.Zx = c1_Zx;
    m.conv2.Zout = c1_Zo;
    ...
    m.fc.W = fc_w;
    m.fc.b = fc_b;
    m.fc.csd_sign = fc_csd_sign;
    m.fc.csd_shift = fc_csd_shift;
    m.fc.shift = fc_s;
    m.fc.Zx = fc_Zx;
    m.fc.Zout = fc_Zo;
...
static int64_t shift_signed_i32(int32_t x, int sh)
{
    int64_t v = (int64_t)x;
    if (v < 0) return -((-v) << sh);
    return v << sh;
...
static int32_t requantize_csd(int32_t acc, const LayerInt8 *layer, int ch)
{
    int64_t tmp = 0;

    for (int t = 0; t < CSD_TERMS; t++) {
        int64_t term = shift_signed_i32(acc, layer->csd_shift[ch][t]);
        if (layer->csd_sign[ch][t] >= 0) tmp += term;
        else tmp -= term;
    }

    return (int32_t)(tmp >> (MULT_FRAC_BITS + layer->shift[ch]));
}
    ...
    for (int oc = 0; oc < out_c; oc++) {
        for (int oh = 0; oh < out_h; oh++) {
            for (int ow = 0; ow < out_w; ow++) {
                ...
                int32_t q = requantize_csd(acc, layer, oc);
...
static void linear_int8(
    ...
    for (int o = 0; o < out_size; o++) {
        ...
        int32_t q = requantize_csd(acc, layer, o);
